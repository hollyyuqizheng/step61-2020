...
/**
 * Models request that are sent to the algorithm. It can be used for both creation and import flow.
 */
public class ScheduleRequest {
   ...
   * @param workHoursStartTimeString: a string representation of the work hours start time. The
   *     string is in format YYYY-MM-DDTHH:MM:SSZ
   * @param workHoursEndTimeString: a string representation of the work hours end time. The string's
   *     format is the same as startTimeString's. All of these fields are required for a request
   *     however, events and tasks can be. empty in the case where there are no events or no tasks.
   *     For example an empty schedule for that day or the user hits start scheduling before they
   *     add any tasks (maybe by accident).
   */
  public ScheduleRequest(
      Collection<CalendarEvent> events,
      Collection<Task> tasks,
      String workHoursStartTimeString,
      String workHoursEndTimeString) {
    if (events == null) {
